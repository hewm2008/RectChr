...
if ($#ARGV==3)
{
	...
	while(<IB>)
	{
		...
		else
		{
			...
			if ($sampleList eq "") {$sampleList=$inf[0];}
			else  {$sampleList=$sampleList.",$inf[0]";}
			$Count++;
		}
	}
	close IB;
...
else
{
	foreach my $k (9..$#head)
	{
		$hashSample{$Count}=$k;
		$Count++;
		if ($sampleList eq "") {$sampleList=$head[$k];}
		else  {$sampleList=$sampleList.",$head[$k]" ;}
	}
}
$Count--;
...
SetParaFor= global
colormap_conf= Region.col


SetParaFor = global
File1 = $OutPut.genotype
show_columns= File1:4
chr_spacing_ratio=0      # Chr之间取消空隙
colormap_conf = $OutPut.col
title="Region $ARGV[1] genotype"
chr_order=$sampleList
#chr_order = A8, A9...A20,SpeSample1.B1,....B20,SpeSample2,...  ## 这儿定义顺序，样品过多 我这只例示
## ## 这儿定义顺序，样品过多 我这只例示   亚群过滤 插入一个or多个空白的样品


SetParaFor= trackALL
chr_label_size_ratio=0.5

SetParaFor= track1
plot_type= heatmap
track_height=$ChrWidth           #  样品过多，400个样品    2.5*400 即画布深度为1000  ，可以用body=1200 /样品数定义chr的宽度
background_color="#FFFFFF"
bg_stroke_width=0
#stroke-width=$strokeWidth
#colormap_legend_size=$SizeGradienRatio
colormap_legend_shift_x=$AAA
#colormap_legend_shift_y=-$BBB
