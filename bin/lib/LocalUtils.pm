...
sub svg2PNGfunction
{
	...
	# PNG conversion is optional:
	# 1) RECTCHR_NO_PNG env (set by the GUI, which renders the SVG itself) disables it
	# 2) on Windows the system convert.exe is the NTFS tool, not ImageMagick -> never probe there
	if ( (defined $ENV{RECTCHR_NO_PNG}) && ($ENV{RECTCHR_NO_PNG} ne "0") )
	{
		print "RECTCHR_NO_PNG is set, skip SVG ---> PNG conversion (SVG file is complete).\n";
		return $OutPutHere;
	}

	print "convert   SVG ---> PNG ...\n";

	if (  ( $arryStat[7]   <  500000000 )  &&  ($^O !~ /MSWin32/i) )
	{
		if ( !(-e $convert) )
		{
			my $wc = `which convert  2> /dev/null `;
			$convert = $wc if defined $wc;
			chomp $convert if defined $convert;
		}
		if ( (-e $convert) )
		{
	...
	if  ( (-e $SVG2XXX) && ($^O !~ /MSWin32/i) )
	{
		system ("  perl   $SVG2XXX    $OutPutHere   " );
		return $OutPutHere;
	}

	print "Info: Can't find [ convert ] (or not supported on this OS), the SVG result is complete.\n";
	print "      For PNG you can either install ImageMagick, or run:  convert  $OutPutHere    $bbb  \n";
	return $OutPutHere;
...
sub plot_histogram {
	...
	{
	   $HashConfi_ref->{$Level}{"colormap_legend_show"} = 0;
	}
	...
	elsif (($NumPlotArry == 2)  &&  (!exists $HashConfi_ref->{$Level}{"hist_No_UD"}) )
	{
		if ((exists $HashConfi_ref->{$Level}{"yaxis_tick_show"}) && ($HashConfi_ref->{$Level}{"yaxis_tick_show"} > 0)) {
	...
	else
	{
		...
		$NumPlotArry--;
		foreach my $AA (0..$NumPlotArry)
		{
		
		my $NowPlot = $PlotInfo_ref->[$AA];
		my $FileNow = $NowPlot->[0];
		my $CoumnNow = $NowPlot->[1];
		my $StartCount = 0;
		...
		for (; $StartCount < $FileRow_ref->[$FileNow]; $StartCount++) 
		{
			...
			$svg->polygon(
				%$path,
				style => {
					'fill'           => $ColorGradientArray_ref->[$AA],
					'stroke'         => $ColorGradientArray_ref->[$AA],
					'stroke-width'   => $HHstrokewidth,
					'stroke-opacity' => $HashConfi_ref->{$Level}{"stroke-opacity"},
					'fill-opacity'   => $HashConfi_ref->{$Level}{"fill-opacity"},
				},
