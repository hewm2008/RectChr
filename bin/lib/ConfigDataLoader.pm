...
########### Open a plain or gzipped file portably (no external gzip needed)###########
sub open_input_file {
	my ($filename) = @_;
	if ($filename =~ /\.gz$/i) {
		my $fh = IO::Uncompress::Gunzip->new($filename)
			or log_message('error', "Can't open gzipped file: $filename : $GunzipError");
		return $fh;
	}
	open my $fh, '<', $filename or log_message('error', "Can't open file: $filename : $!");
	return $fh;
}
		...
		if (!exists $MAX_COLOR_COUNT{$color_name}) 
		{
			...
			if (-e $file)
			{
				...
				if (open my $cfh, '<', $file) {
					...
					log_message('warn', "Can't open color palette file: $file : $!");
				}
				chomp $count;
